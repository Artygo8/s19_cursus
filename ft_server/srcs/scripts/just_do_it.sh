...
apt install -y sudo
apt install -y nginx
apt install -y php7.3-fpm php-curl php-intl php-mbstring \
...
sudo ./mkcert -install
sudo ./mkcert -cert-file /etc/ssl/certs/localhost.crt \
	-key-file /etc/ssl/certs/localhost.key localhost 127.0.0.1 localhost.com
